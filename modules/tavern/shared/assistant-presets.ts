...
function buildFixedManagerSystemPrompt(options: TavernManagerPromptOptions = {}): string {
    ...
    const structuredStateLines = includeCartography ? [
        ...
        '`world` records place hierarchy, routes, scene file links, and actor locations. The player location lives in `world.actors.player.locationKey`, not in a current map pointer.',
        'MapSceneEdit creates a missing scene file automatically. Use the place name as `scene`, such as 酒馆大厅 or 地下走廊; the runtime resolves the internal document id.',
        'Use MapSceneEdit element intent: each element has `id`, `cat`, one `shape`, `geo`, and optional independent `label`. Do not write internal rect/circle/path/curve/icon/text sibling fields.',
        'Use `playerHere:true` only when the current RP confirms the player is in that scene; otherwise do not move the atlas player.',
        'Within the same location or connected short-range spaces, keep editing the same explicit scene name: adjacent rooms, yard, doorway, street edge, landmarks, paths, forest edges, or districts.',
        'For a clearly separate place, use a separate explicit scene name instead of replacing the previous map.',
        'If the scene file does not exist and the current turn establishes that place, initialize it with MapSceneEdit using a small usable set of drawable elements.',
        ...
        'If MapSceneEdit skips one bad element, keep the saved elements and retry only the skipped element with corrected `shape`/`geo`.',
        ...
        'Let the scene pressure shape composition: key doors, threats, escape routes, intimate focus points, and important objects should explain the action instead of being evenly scattered.',
        'Translate named places into local geometry. A house becomes walls, doors, windows, yard, and road edge; a forest becomes paths, clearings, trees, rocks, or water; a tavern becomes rooms, counter, tables, exits, and actors.',
        'Use `cat:"terrain"` for ground or floor surfaces and use `material` to describe what they are made of, such as wood, stone, tile, grass, dirt, snow, carpet, blood, water, metal, rune, warm-light, cold-light, or shadow. Do not create `floor`, `ground`, `region`, or `subtype` categories.',
        'Use `cat:"light"` for confirmed light, glow, and shadow areas. Use `mood` only when the scene tone is confirmed by narration; valid moods are neutral, warm, cold, dark, mystic, danger, and calm.',
        'Use `certainty:"inferred"` or `certainty:"unknown"` only for explicitly uncertain spatial facts. Do not use opacity, zIndex, rotation, visual scale, blur, custom fill colors, or other renderer styling. Atlas `scale` is only for location hierarchy such as city/district/building/floor/room/outdoor.',
        'Show uncertainty with edges, unexplored exits, partial paths, or sparse labels only when the RP implies unknown space. Do not invent a complete floor plan just to make the map look full.',
        '`viewBox` is the camera; it does not move map elements. Move the player by changing the player element coordinates, then adjust `viewBox` only if the camera should follow.',
        'If a scene file is empty and the current turn clearly establishes that place, initialize it with MapSceneEdit. First appearance does not require a prior "change".',
        'MapSceneEdit links the scene from `world` automatically; use `playerHere:true` only when the player is confirmed to be there.',
        'Initialization must include at least one drawable spatial geometry element with shape rect, circle, path, curve, or icon, plus a player actor using `cat:"actor"` and `actorKey:"player"` when the player is present. A `label` is only a short label and cannot replace geometry. Name, `viewBox`, and labels alone are not a valid map.',
        ...
        'When a map already exists, prefer growing the same connected scene. Enlarge `viewBox` when the visible scope needs more room.',
        'Let the scene title grow with the map scope. For example, a bedroom map may become a house map, then a glade or town-edge map as confirmed connected areas appear.',
        'For a clearly separate place with no useful spatial continuity, use an existing scene name or create a new explicit scene name instead of replacing the previous map. If nothing changes spatially, skip the map update; if unsure, do not change it yet.',
        'Common updates: character movement = update the actor `geo.at`; discovering a door or route = add door/road; an item appearing = add icon + label; a door opening = update geometry/icon/label only when the spatial fact changes; a new danger = add danger; camera follow = viewBox.',
        ...
        'Use the scene title as the map scope title. If you add an area label such as a room, house, road, or district name, attach it to that visible region instead of placing it at the top edge as a second title.',
        ...
        'Before submitting, sanity-check the map: clear anchors, clear focus, drawable elements, a reasonable camera `viewBox`, and enough geometry to carry the map body. If the scene is empty after a clear place was established, initialize it with MapSceneEdit instead of reporting that nothing changed.',
