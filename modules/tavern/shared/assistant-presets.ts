...
function buildFixedManagerSystemPrompt(options: TavernManagerPromptOptions = {}): string {
    ...
    const structuredStateLines = includeCartography ? [
        ...
        'First-map rule: when a clear place is established and its scene is empty, create a small usable map instead of skipping. Include the scene boundary/main surface first, then the player actor if present, and one to three confirmed anchors.',
        ...
        'Scene-map construction order: first define the visible scope and camera, then draw the outer boundary or main surface expressively along the real visible shape, then place internal zones, doors, furniture, hazards, objects, labels, and actors relative to that structure.',
        'Closed or contained scenes must have an outer boundary before interior details. Trace enclosing walls, edges, shells, shorelines, clearing edges, and other limits along their true silhouette; use a simple rect only when the boundary is truly simple, and use `path` or `curve` whenever the real outline bends, narrows, breaks, or has an organic/irregular shape.',
        ...
        'Indoor and vehicle scenes usually start from an outer-wall/hull rect or outline plus a floor/terrain surface; outdoor scenes from a clearing/yard/shore/path/ground surface; passages from boundary lines and a stretched `viewBox`.',
