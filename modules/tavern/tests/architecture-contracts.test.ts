...
test('tavern streaming action-check UI renders from live runtime events and keeps dark card styling aligned', () => {
    ...
    assert.match(conversationPanelSource, /@click="clearMessageActionTray\(\); closeComposeMenu\(\)"/);
    ...
    assert.match(conversationPanelSource, /System[\s\S]*User[\s\S]*Assistant/);
    assert.match(conversationPanelSource, /参与世界书扫描/);
    assert.match(conversationPanelSource, /v-model="authorNoteDraft\.prompt"/);
    assert.match(conversationPanelSource, /v-model\.number="authorNoteDraft\.depth"/);
    assert.match(conversationPanelSource, /v-model\.number="authorNoteDraft\.interval"/);
    assert.doesNotMatch(conversationPanelSource, /@input="updateAuthorNote(?:Prompt|Depth|Interval)"/);
