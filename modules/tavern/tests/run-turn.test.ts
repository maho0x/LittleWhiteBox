...
test('xb tavern run turn can trigger manager summary with delegate config', async () => {
    ...
    assert.match(managerPrompt, /State tools maintain two persistent spatial layers/i);
    assert.match(managerPrompt, /`tavern\.atlas\/main` is the world topology/i);
    assert.match(managerPrompt, /Move the player between locations only with `move-actor`/i);
    assert.match(managerPrompt, /Use map `activate:true` only to switch or maintain the map tool/i);
    ...
    assert.match(managerPrompt, /Create\/switch docs only for clearly named/i);
